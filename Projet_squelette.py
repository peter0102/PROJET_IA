...
def alpha_beta_decision(board, turn, ai_level, queue, max_player):
    ...
    isMaximizing=None
    best_value = -np.inf
    if max_player == 1: #le joueur 1 maximise
        isMaximizing = True
    if max_player == 2: #le joueur 2 minimise
        isMaximizing = False
    moves = board.get_possible_moves()
    for move in moves:
        new_board = board.copy()
        new_board.add_disk(move, max_player, False)
        new_score = minimax(new_board, turn, max_player, ai_level, isMaximizing, -np.inf, np.inf)
...
def minimax(board, turn, player, depth, isMaximizingPlayer, alpha, beta):
    ...
    if (isMaximizingPlayer):
        ...
        for move in moves:
            ...
            new_score = minimax(new_board, turn+1, 2-(turn%2), depth - 1, False, alpha, beta)
        ...
        for move in moves:
            new_board = board.copy()
            new_board.add_disk(move, player, False)
            new_score = minimax(new_board, turn+1, 2-(turn%2), depth - 1, True, alpha, beta)
...
class Board: #plus de valeur à la défense qu'à l'attaque
